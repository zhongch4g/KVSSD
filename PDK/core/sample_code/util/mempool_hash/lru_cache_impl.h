...
#include <string.h>
/***
 * https://github.com/Stand1210/c-LRU-/tree/master
*/

/*定义LRU缓存的缓存单元*/
typedef struct cacheEntryS {
	uint64_t key; /* 数据的key */
	uint64_t data; /* 数据的data */
...
/*将数据放入LRU缓存中*/
int LRUCacheSet(void *lruCache, uint64_t key, uint64_t data);
/*从缓存中获取数据*/
uint64_t LRUCacheGet(void *lruCache, uint64_t key);
/*打印缓存中的数据， 按访问时间从新到旧的顺序输出*/
void LRUCachePrint(void *lruCache);

void getTailFromList(LRUCacheS *cache, uint64_t *key, uint64_t *value);
...
******************************/
/* 创建一个缓存单元 */
static cacheEntryS *newCacheEntry(uint64_t key, uint64_t data)
{
...
/*哈希函数*/
static unsigned int hashKey(LRUCacheS *cache, uint64_t key)
{
	unsigned int len = 8;
...
/*从哈希表获取缓存单元*/
static cacheEntryS *getValueFromHashMap(LRUCacheS *cache, uint64_t key)
{
	/*1.使用哈希函数定位数据存放于哪个槽*/
...
static void removeEntryFromHashMap(LRUCacheS *cache, cacheEntryS *entry)
{
	...
	/*1.使用哈希函数定位数据位于哪个槽*/
	n = cache->hashMap[hashKey(cache, entry->key)];
...
void getTailFromList(LRUCacheS *cache, uint64_t* key, uint64_t* value)
{
	...
	*key = cache->lruListTail->key;
	*value = cache->lruListTail->data;
}
