...
DEFINE_uint64 (key_size, 8, "The key size");
...
DEFINE_string (path, "/mnt/nvmevirt/rocksdb_temp", "The path of the rocksdb");
...
public:
    ...
    Benchmark ()
        : num_ (FLAGS_num),
          ...
          writes_ (FLAGS_write),
          key_trace_ (nullptr) {
        uint64_t cache_size = 10ULL * 1024 * 1024 * 1024;
        // woptions.disableWAL = true;
        woptions.sync = false;
        // -------------------------------------------------------------------------------------
        options.use_direct_reads = true;
        options.use_direct_io_for_flush_and_compaction = true;
        options.db_write_buffer_size = 0;  // disabled
        options.create_if_missing = true;
        options.manual_wal_flush = true;
        options.compression = rocksdb::CompressionType::kNoCompression;
        // options.row_cache = rocksdb::NewLRUCache(1 * 1024 * 1024 * 1024);
        ...
        // create the database
        rocksdb::Status s = rocksdb::DB::Open (options, FLAGS_path, &db);
    }
    ...
    void DoWrite (ThreadState* thread) {
        ...
        uint64_t klen = FLAGS_key_size;
        uint64_t vlen = FLAGS_value_size;
        ...
        while (key_iterator.Valid ()) {
            ...
            for (; j < batch && key_iterator.Valid (); j++) {
                ...
                auto ret = db->Put (woptions, rocksdb::Slice ((char*)kvskey.key, kvskey.length), \
                                    rocksdb::Slice ((char*)kvsvalue.value, kvsvalue.length));
                if (!ret.ok()) {
    ...
    void DoReadAll (ThreadState* thread) {
        ...
        while (!duration.Done (batch) && key_iterator.Valid ()) {
            ...
            for (; j < batch && key_iterator.Valid (); j++) {
                ...
                std::string rValue;
                auto ret = db->Get (roptions, rocksdb::Slice ((char*)kvskey.key, kvskey.length),
                                     &rValue);
                if (!ret.ok ()) {
                    not_find++;
                }
