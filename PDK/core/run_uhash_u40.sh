...
ulimit -Sn 204800
sudo ./mempool_hash_application --path=/mnt/nvmevirt/mempool_hash --benchmarks=load --worker_threads=${nthreads} --num=${num} --key_size=${ksize} --value_size=${vsize} --report_interval=1 --batch=100 | tee ${result_path}/${prefix}_${file_name}.data
